...
def exponential_moving_average(data, window_size, smoothing=2):
    return data.rolling(window=window_size, min_periods=1).mean() ** smoothing


# Calculate the Moving Average for each numeric column (20 data points)
#moving_average_data = sorted_data.iloc[:, 1:].rolling(window=20, min_periods=1, center=True).mean()
window_size = 20
smoothing = 0.5

moving_average_data = sorted_data[numeric_columns].apply(exponential_moving_average, window_size=window_size, smoothing=smoothing)
...
for i, col in enumerate(numeric_columns):
    fig.add_trace(go.Scatter(
        ...
        y=moving_average_data[col],
        mode='lines',
        name=f'{col} Moving Average',
        line=dict(color=colors[i]),  # Use a different color for each column
...
# Show the graph
pio.write_html(fig, 'movingAverageLinePlot#3.html')
pio.show(fig)
