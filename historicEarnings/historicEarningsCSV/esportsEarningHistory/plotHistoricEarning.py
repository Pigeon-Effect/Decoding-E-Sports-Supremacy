...
for country in combined_df.sort_values('rank')['country'].unique():
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertemplate='Country: %{text}<br>Year: %{x}<br>Earning: $%{y:,.2f}',
...
print(combined_df)
# Show the graph
pio.write_html(fig, 'historicYearlyEarningsByCountryPE.html')
pio.show(fig)
offline.plot(fig, filename='HistoricEarningsPigeonEffect.html', auto_open=False)
